...
public:
    ...
    virtual void cycle(CPU *cpu) {assert(false);}
    virtual bool isExecutionFinished() {assert(false);}
 
	virtual bool isRegPckEmpty() { assert(false); }
	...
	virtual unsigned long long get_Instruction_S() { assert(false); }

	virtual unsigned int get_global_cycle() {}
	map<unsigned int, address_t > in_fmap_buffer; //save input feature map, 
