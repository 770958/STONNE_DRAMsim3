...
public:
    ...
    virtual void printConfiguration(std::ofstream& out, unsigned int indent) { assert(false); }  //This function prints the configuration of the ASNetwork (i.e., ASwitches configuration such as ADD_2_1, ADD_3_1, etc)
    virtual void printStats(std::ofstream& out, unsigned int indent) { assert(false); } //This functions prints the statistics obtained during the execution. 
    virtual void printEnergy(std::ofstream& out, unsigned int indent){ assert(false); }
	virtual	unsigned long long  get_Instruction() { assert(false); }
	virtual unsigned int get_cycle(){ assert(false); }
	virtual unsigned int get_global_cycle() { assert(false); }
