...
private:
    ...
    void cycle(string layer_name);
	void mem_partition_cycle(CPU *cpu);
    void printStats();
	void printSystemStats(CPU *cpu, string layer_name, string index_layer_);
	...
	//system stats
	unsigned long mem_load_num = 0;	    //与DRAM交互的load指令数
	unsigned long mem_store_num = 0;	//与DRAM交互的store指令数

	bool execution_finished = false;
	bool cpu_finished = false;
 
	ASwitchStats get_AS_Instruction_num;
   
public:
    Stonne (Config_stonne stonne_cfg);
    ~Stonne();
