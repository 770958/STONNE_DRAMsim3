...
private:
	...
	unsigned long long store_instruction;
	...
	bool send_finished; //Flag that indicates all data fetches completed.
	...
	std::queue<DataPackage*> reg_pck;

	//Fifo* test = new Fifo(512);
	std::string addr_genel;
	std::string addr_mac;
	std::vector<std::string> addr_bmt;
	std::string addr_ctr;

	std::string reuse_type;
	unsigned int on_chip_size;
public:
	SparseSDMemory(id_t id, std::string name, Config_stonne stonne_cfg, Connection* write_connection);
	~SparseSDMemory();
	...
	unsigned long long get_Instruction_S();

	unsigned int get_global_cycle();
	bool isRegPckEmpty() { return this->reg_pck.size() == 0; }
