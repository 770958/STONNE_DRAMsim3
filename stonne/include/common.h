...
struct Transaction {
    ...
    Transaction(uint64_t addr, bool is_write)
        : addr(addr),
          ...
          is_write(is_write),
          addr_str(""){}
    Transaction(uint64_t addr, bool is_write, std::string addr_str)
        : addr(addr),
          added_cycle(0),
          complete_cycle(0),
          is_write(is_write),
    ...
    friend std::stringstream& operator>>(std::stringstream& is, Transaction& trans);
