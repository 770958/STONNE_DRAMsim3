...
class BaseDRAMSystem {
   public:
    ...
    virtual bool AddTransaction(uint64_t hex_addr, bool is_write,std::string addr_str) = 0;

    virtual void ClockTick() = 0;
    int GetChannel(uint64_t hex_addr) const;

	virtual std::queue<int> ReturnDelay() = 0;
	virtual void FreeDelayQueue() = 0;
    std::function<void(uint64_t req_id)> read_callback_, write_callback_;
    static int total_channels_;
	double total_delay = 0.00;
...
class JedecDRAMSystem : public BaseDRAMSystem {
   public:
    ...
    bool AddTransaction(uint64_t hex_addr, bool is_write,std::string addr_str) override;
    void ClockTick() override;
	std::queue<int> ReturnDelay() override;
	void FreeDelayQueue()override;
};
