...
ASNetwork::ASNetwork(id_t id, std::string name, Config_stonne stonne_cfg, Connection* outputConnection) : ReduceNetwork(id, name) {
...
void ASNetwork::cycle() {
    ...
    if(this->accumulation_buffer_enabled > 0) {
		...
		this->as_num += this->accumulationBuffer->get_Instruction();
		this->as_cycle += this->accumulationBuffer->get_cycle();
    }
    ...
    ASwitch* as = aswitchtable[current_switch_pair];
    as->cycle();
	this->as_num += as->get_Instruction();
	this->as_cycle += as->get_cycle();
	this->as_global_cycle = as->get_global_cycle();
        ...
        while(j < switches_this_level) {
            ...
            if(as->isFwEnabled()) { //If there is fw link in this node
                ...
                if(as->getFlDirection() == SEND) { //If the current one is who send the data, the next goes first since it has to take the data in the next cycle
                    as_next->cycle();
					this->as_num += as_next->get_Instruction();
					this->as_cycle += as_next->get_cycle();
					this->as_global_cycle = as_next->get_global_cycle();
...
unsigned long long ASNetwork::get_Instruction()
{
	return this->as_num;
}
unsigned int ASNetwork::get_cycle() {
	return this->as_cycle;
}

unsigned int ASNetwork::get_global_cycle()
{
	return this->as_global_cycle;
}
