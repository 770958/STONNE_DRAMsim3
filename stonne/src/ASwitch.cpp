...
ASwitch::ASwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config_stonne stonne_cfg) : Unit(id, name) {
    this->level = level;
...
ASwitch::ASwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config_stonne stonne_cfg, Connection* inputLeftConnection,Connection* inputRightConnection, Connection* forwardingConnection, Connection* outputConnection, Connection* memoryConnection) : ASwitch(id, name, level, num_in_level, stonne_cfg) { //Constructor
...
void ASwitch::send() {
   ...
   //     std::cout << "DEBUG GENERAL " << this->level << ":" << this->num_in_level << " at cycle " << this->local_cycle << std::endl;
        ...
        if(this->forward_to_memory) { //Optimization to send the psum to the memory once it is completed without traverse other adders con fw configuration
			...
			for (int i = 0; i < vector_to_send_parent.size(); i++) {
				vector_to_send_parent[i]->data_type = OACTIVATION;
			}
...
void ASwitch::receive_childs() { 
    if(this->inputLeftConnection->existPendingData()) { //If there is data to receive on the left
        ...
        for(int i=0; i<data_received_left.size(); i++) {
			...
			//this->local_cycle += data_received_left[i]->getDelay();	//waiting for dram_data
         }
    }
    if(this->inputRightConnection->existPendingData()) {
        ...
        for(int i=0; i<data_received_right.size(); i++) {
			...
			//this->local_cycle += data_received_right[i]->getDelay();	//waiting for dram_data
         }
    }
/*
        ...
        if(this->forwardingConnection->existPendingData()) {
           ...
           /* if((level == 5) && (num_in_level==21)) {
            ...
            for(int i=0; i<data_received_fw.size(); i++) {
				...
				//this->local_cycle += data_received_fw[i]->getDelay();	//waiting for dram_data
             }
        }
         /*if((level == 5) && (num_in_level == 21)) {
...
unsigned long long ASwitch::get_Instruction()
{
	return this->as_num;
}
unsigned int ASwitch::get_cycle()
{
	return this->local_cycle;
}

unsigned int ASwitch::get_global_cycle()
{
	return this->aswitchStats.total_cycles;
}
