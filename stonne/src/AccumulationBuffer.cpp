...
AccumulationBuffer::AccumulationBuffer(id_t id, std::string name, Config_stonne stonne_cfg, unsigned int n_accumulators) : Unit(id, name) {
...
void AccumulationBuffer::cycle() {
	...
	for(int i=0; i<this->n_accumulators; i++) {
	  ...
	  //this->acc_num = accumulatortable[i]->get_Instruction();
	  //this->acc_cycle = accumulatortable[i]->get_cycle();
	  this->acc_num = std::max(this->acc_num, accumulatortable[i]->get_Instruction());;
	  this->acc_cycle = std::max(this->acc_cycle, accumulatortable[i]->get_cycle());

	//  this->acc_cycle = index_c > this->acc_cycle ? index_c : this->acc_cycle;
	//  this->acc_num = index_n > this->acc_num ? index_n : this->acc_num;
	} 
	//std::cout << this->acc_num << std::endl;
}
...
unsigned long long AccumulationBuffer::get_Instruction()
{
	return this->acc_num;
}

unsigned int AccumulationBuffer::get_cycle()
{
	return this->acc_cycle;
}

unsigned int AccumulationBuffer::get_global_cycle()
{
	return this->acc_global_cycle;
}
