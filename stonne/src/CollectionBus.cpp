...
Bus::Bus(id_t id, std::string name, Config_stonne stonne_cfg) : Unit(id, name) {
...
unsigned long long Bus::get_Instruction()
{
...
void Bus::cycle(CPU *cpu) { 
	this->store_instruction = 0;
    for(int i=0; i<collection_bus_lines.size(); i++) {
        collection_bus_lines[i]->cycle(cpu);
		this->store_instruction += collection_bus_lines[i]->get_Instruction();
    }
}
