...
Config_stonne::Config_stonne() {
    this->reset();
}

void Config_stonne::loadFile(std::string config_file) {
	...
	//added by wangjs -- reuse type && on-chip mem size
	auto sdmemory_reuse_type = config->get_qualified_as<std::string>("SDMemory.reuse");  //IS WS OS
	if (sdmemory_reuse_type) {
		this->m_SDMemoryCfg.reuse_type = *sdmemory_reuse_type;
	}

	auto sdmemory_on_chip_mem = config->get_qualified_as<unsigned int>("SDMemory.mem_size");  //SIZE
	if (sdmemory_on_chip_mem) {
		this->m_SDMemoryCfg.on_chip_size = *sdmemory_on_chip_mem;
	}
	//added end
...
// ---------------------------------------------------------
    ...
    m_SDMemoryCfg.port_width=64;
	m_SDMemoryCfg.reuse_type = "IS";
	m_SDMemoryCfg.on_chip_size = 1024;

}

bool Config_stonne::sparsitySupportEnabled() {
    return m_SDMemoryCfg.mem_controller_type==SIGMA_SPARSE_GEMM; //If the controller is sparse, then sparsity is allowed
}
bool Config_stonne::convOperationSupported() {
...
// -----------------------------------------------------------------------------------------------
void Config_stonne::printConfiguration(std::ofstream& out, unsigned int indent) {
