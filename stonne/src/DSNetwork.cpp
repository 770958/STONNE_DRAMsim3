...
DSNetwork::DSNetwork(id_t id, std::string name, Config_stonne stonne_cfg, unsigned int ms_size, Connection* inputConnection) : Unit(id,name) {
...
void DSNetwork::cycle( ) {
    ...
    for(int i=0; i < this->nlevels; i++) {
        for(int j=0; j < switches_this_level; j++) {
			...
			ds_cycle += ds->get_cycle();
			ds_num += ds->get_Instruction();
			ds_global_cycle = ds->get_global_cycle();
        }
		switches_this_level = switches_this_level * 2;
    }
...
unsigned long long DSNetwork::get_Instruction()
{
	return this->ds_num;
}

unsigned int DSNetwork::get_cycle()
{
	return this->ds_cycle;
}

unsigned int DSNetwork::get_global_cycle()
{
