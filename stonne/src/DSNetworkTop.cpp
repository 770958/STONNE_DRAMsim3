...
DSNetworkTop::DSNetworkTop(id_t id, std::string name, Config_stonne stonne_cfg) : DistributionNetwork(id,name) {
...
void DSNetworkTop::cycle( ) {
    ...
    for(int i=0; i<this->n_input_ports; i++) {
        ...
        dsnetworks[i]->cycle( );
		this->ds_cycle += dsnetworks[i]->get_cycle();
		this->ds_num += dsnetworks[i]->get_Instruction();
		this->ds_global_cycle = dsnetworks[i]->get_global_cycle();
    }
}
...
unsigned long long DSNetworkTop::get_Instruction()
{
	return this->ds_num;
}

unsigned int DSNetworkTop::get_cycle()
{
	return this->ds_cycle;
}

unsigned int DSNetworkTop::get_global_cycle()
{
	return this->ds_global_cycle;
}
