...
DSwitch::DSwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config_stonne stonne_cfg, unsigned int ms_size) : Unit(id, name)  { //ms_size depends on the local ms_size in the subtree
    this->level = level;
	...
	this->ds_cycle = 0;
	this->ds_num = 0;
	this->mc = stonne_cfg.m_SDMemoryCfg.mem_controller_type;

}

DSwitch::DSwitch(id_t id, std::string name, unsigned int level, unsigned int num_in_level, Config_stonne stonne_cfg, unsigned int ms_size,
...
unsigned long long clk_last = 0;
unsigned long long clk_now = 0;
...
void DSwitch::route_packages() { //TODO It is supposing you have enouth bandwidth in the connection
    ...
    for(int i=0; i<this->data.size(); i++) { //For every package in the connection
		...
		this->ds_num++;
		this->ds_cycle += pck->getDelay();
		//std::cout << "ds read  delay: " << this->data[i]->getDelay() << std::endl;
...
void DSwitch::cycle( ) {
	...
	//this->ds_cycle++;
	//std::cout << "ds cycle 0: " << ds_cycle << std::endl;
	...
	//std::cout << this->pending_data << std::endl;
    if(this->pending_data) {//If there is data to send 
        this->route_packages();
    }
	//std::cout << "ds cycle 1: " << ds_cycle << std::endl;
...
unsigned long long DSwitch::get_Instruction()
{
	return this->ds_num;
}

unsigned int DSwitch::get_cycle()
{
	return this->ds_cycle;
}

unsigned int DSwitch::get_global_cycle()
{
	return this->dswitchStats.total_cycles;
}
