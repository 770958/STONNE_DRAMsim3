...
FENetwork::FENetwork(id_t id, std::string name, Config_stonne stonne_cfg, Connection* outputConnection) : ReduceNetwork(id, name) {
...
void FENetwork::cycle() {
    ...
    as_extra->cycle();
	this->as_num += as_extra->get_Instruction();
	this->as_cycle += as_extra->get_cycle();
	this->as_global_cycle = as_extra->get_global_cycle();
...
unsigned long long FENetwork::get_Instruction()
{
	return this->as_num;
}

unsigned int FENetwork::get_cycle()
{
	return this->as_cycle;
}

unsigned int FENetwork::get_global_cycle()
{
