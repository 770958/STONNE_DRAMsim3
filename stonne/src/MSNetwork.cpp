...
MSNetwork::MSNetwork(id_t id, std::string name, Config_stonne stonne_cfg) : MultiplierNetwork(id, name) {
...
void MSNetwork::cycle() {
    ...
    for(int i=0; i < this->ms_size; i++) {
        ...
        ms->cycle();
		this->ms_num += ms->get_Instruction();
		this->ms_cycle += ms->get_cycle();
		this->ms_global_cycle = ms->get_global_cycle();
    }
}
...
unsigned long long MSNetwork::get_Instruction()
{
	return this->ms_num;
}

unsigned int MSNetwork::get_cycle()
{
	return this->ms_cycle;
}

unsigned int MSNetwork::get_global_cycle()
{
	return this->ms_global_cycle;
}
