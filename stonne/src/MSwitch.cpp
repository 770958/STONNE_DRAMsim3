...
MSwitch::MSwitch(id_t id, std::string name, int num, Config_stonne stonne_cfg) :
Unit(id, name){
    this->num = num;
...
MSwitch::MSwitch(id_t id, std::string name, int num, Config_stonne stonne_cfg, Connection* outputConnection, Connection* inputConnection) : MSwitch(id, name, num, stonne_cfg) {
...
unsigned long long  MSwitch::get_Instruction()
{
	return	this->ms_num;
}

unsigned int MSwitch::get_cycle()
{
	return this->local_cycle;
}

unsigned int MSwitch::get_global_cycle()
{
