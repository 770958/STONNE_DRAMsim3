...
MultiplierOS::MultiplierOS(id_t id, std::string name, int row_num, int col_num, Config_stonne stonne_cfg):
Unit(id, name){
...
MultiplierOS::MultiplierOS(id_t id, std::string name, int row_num, int col_num, Config_stonne stonne_cfg, Connection* left_connection, Connection* right_connection, Connection* top_connection, Connection* bottom_connection) : MultiplierOS(id, name, row_num, col_num, stonne_cfg) {
...
//    std::cout << "MS: " << this->num << ". weight fifo size: " << weight_fifo->size() << " N_Folding: " << n_folding << std::endl;
    ...
    if((!left_fifo->isEmpty()) && (!top_fifo->isEmpty())) { //If both queues are not empty
			...
			//local_cycle += pck_result->getDelay();
...
unsigned long long MultiplierOS::get_Instruction()
{
	return	this->ms_num;
}

unsigned int MultiplierOS::get_cycle()
{
	return this->local_cycle;
}

unsigned int MultiplierOS::get_global_cycle()
{
	return this->mswitchStats.total_cycles;
}
