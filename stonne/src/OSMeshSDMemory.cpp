...
OSMeshSDMemory::OSMeshSDMemory(id_t id, std::string name, Config_stonne stonne_cfg, Connection* write_connection) : MemoryController(id, name) {
...
//added by wangjs
string _dec2hex_(long long int i) //将int转成16进制字符串
{
...
unsigned long long clk_from_dram_last2 = 0;
unsigned long long clk_from_dram_now2 = 0;
void OSMeshSDMemory::cycle(CPU *cpu) {
	...
	{	//Initialize these for the first time
		...
		//std::cout <<"cols_used: "<<this->cols_used<<" rows_used: "<<this->rows_used <<std::endl;
	...
	if (current_state == OS_DIST_INPUTS) {
		...
		for (int i = 0; i < this->cols_used; i++) {
			...
			//added by wangjs
			auto it = in_fmap_buffer.find((index_N + i)*this->K + this->current_K);
 			if (it == in_fmap_buffer.end()) {
				in_fmap_buffer.insert(pair<unsigned int, address_t>((index_N + i)*this->K + this->current_K, &(this->KN_address[(index_N + i)*this->K + this->current_K])));

				std::stringstream ss;
				ss << _dec2hex_((long long int)(&(this->KN_address[(index_N + i)*this->K + this->current_K]))) << " " << "READ" << " " << std::to_string(this->sdmemoryStats.total_cycles) << " " << "weight";
		...
		for (int i = 0; i < rows_used; i++) {
			...
			auto it = in_fmap_buffer.find((index_M + i)*this->K + this->current_K);
			...
			if (this->reuse_type == "IS") {
				if (it == in_fmap_buffer.end()) {
					...
					in_fmap_buffer.insert(pair<unsigned int, address_t>((index_M + i)*this->K + this->current_K, &this->MK_address[(index_M + i)*this->K + this->current_K]));

					std::stringstream ss;
					ss << _dec2hex_((long long int)(&(this->MK_address[(index_M + i)*this->K + this->current_K]))) << " " << "READ" << " " << std::to_string(this->sdmemoryStats.total_cycles) << " " << "feature_map";
					std::string result = ss.str();
					cpu->req_fifo.push(result);
					cpu->npu_num++;
...
void OSMeshSDMemory::send() {
	...
	for (int i = 0; i < this->n_read_ports; i++) {
		...
		//If psums fifo is empty then input fifo is checked. If psum is not empty then else do not compute. Important this ELSE to give priority to the psums and do not send more than 1 pck
			...
			/*std::cout << " " << std::endl;
			std::cout << "input fifo to DS" << std::endl;
...
void OSMeshSDMemory::receive(CPU *cpu) { //TODO control if there is no space in queue
...
void OSMeshSDMemory::getInputData(string data)
{
	...
	addr_mac = _dec2hex_(mac_flag.to_ulong() + MAC_DATA_BEGIN);
	//*****CTR*****//
	...
	addr_ctr = _dec2hex_(ctr_flag.to_ulong() + CTR_DATA_BEGIN);
	//*****BMT*****//
	...
	for (int index_i = 0; index_i < 6; index_i++)
	{
		...
		string str_temp2 = _dec2hex_(bmt_flag.to_ullong() + bmt_data_begn[index_i]);
...
unsigned long long OSMeshSDMemory::get_Instruction()
{
	return this->load_instruction;
}

unsigned long long OSMeshSDMemory::get_Instruction_S()
{
	return this->store_instruction;
}


unsigned int OSMeshSDMemory::get_global_cycle()
{
