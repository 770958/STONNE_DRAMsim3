...
void VNAT_Register::update() {
	...
	this->addr = this->base_addr + (index_N)*dnn_layer->get_X_()*dnn_layer->get_Y_()*dnn_layer->get_K()*dnn_layer->get_G()*_word_size + index_X * dnn_layer->get_Y_()*dnn_layer->get_K()*dnn_layer->get_G()*_word_size + index_Y * dnn_layer->get_K()*dnn_layer->get_G()*_word_size + index_G * dnn_layer->get_K()*_word_size + index_K * _word_size;
	//std::cout << "Address: " << this->addr << std::endl;


}

SDMemory::SDMemory(id_t id, std::string name, Config_stonne stonne_cfg, Connection* write_connection) : MemoryController(id, name) {
	...
	this->send_finished = false;
	this->current_output_pixel = 0;
	this->local_cycle = 0;
	this->load_instruction = 0;

	this->on_chip_size = stonne_cfg.m_SDMemoryCfg.on_chip_size;
	this->reuse_type = stonne_cfg.m_SDMemoryCfg.reuse_type;
			...
			for (unsigned k = 0; k < this->current_tile->get_T_K(); k++) {
				for (unsigned x = 0; x < this->current_tile->get_T_X_(); x++) {
					for (unsigned y = 0; y < this->current_tile->get_T_Y_(); y++) {
						...
						unsigned int addr_offset = n * this->dnn_layer->get_G()*this->dnn_layer->get_K()*this->dnn_layer->get_X_()*this->dnn_layer->get_Y_()*_word_size + x * this->dnn_layer->get_Y_()*this->dnn_layer->get_G()*this->dnn_layer->get_K()*_word_size + y * this->dnn_layer->get_G()*this->dnn_layer->get_K()*_word_size + g * this->dnn_layer->get_K() + k * _word_size;
...
void SDMemory::setLayer(DNNLayer* dnn_layer, address_t input_address, address_t filter_address, address_t output_address, Dataflow dataflow) {
   ...
   /*
					...
					address_t current_filter_address = filter_address +  k*filter_size*_word_size;
					address_t current_input_address = input_address + n*input_size*_word_size + x_*row_input_size*_word_size*stride +
																	 y_*dnn_layer->get_C()*_word_size*stride;
					address_t current_output_address = output_address + n*output_size*_word_size + k*channel_output_size*_word_size +
																	 x_*row_output_size*_word_size +  y_*dnn_layer->get_K();
...
//added by wangjs
string SDMemory::dec2hex(long long int i) //将int转成16进制字符串
{
...
unsigned long long clk_from_dram_last1 = 0;
unsigned long long clk_from_dram_now1 = 0;

void SDMemory::cycle(CPU *cpu) {
	//string trace_file = "../dramsim3/output/req.trace";
	//ofstream ofs;						//定义流对象
	//ofs.open(trace_file, ios::out | ios::app);		//以写的方式打开文件 
	...
	if (!this->input_finished && (pck_iteration <= current_iteration)) { //If 
		...
		unsigned window_size = this->current_tile->get_T_R()*this->current_tile->get_T_S()*this->current_tile->get_T_C();	//filter tile size
						...
						for (unsigned c = 0; c < current_tile->get_T_C(); c++) {
							for (unsigned r = 0; r < current_tile->get_T_R(); r++) {
								for (unsigned s = 0; s < current_tile->get_T_S(); s++) {
									...
									data_t data = filter_address[(index_G + g)*this->group_size*_word_size + (index_K + k)*this->filter_size*_word_size + (index_R + r)*this->row_filter_size*_word_size + (index_S + s)*dnn_layer->get_C()*_word_size + (index_C + c)];  //Fetching. Note the distribution in memory is interleaving the channels

									this->load_instruction++;
									...
									//added by wangjs
									if (this->reuse_type == "WS") {
										auto it = in_fmap_buffer.find((index_G + g)*this->group_size*_word_size + (index_K + k)*this->filter_size*_word_size + (index_R + r)*this->row_filter_size*_word_size + (index_S + s)*dnn_layer->get_C()*_word_size + (index_C + c));
										//if this is not in buffer
										if (it == in_fmap_buffer.end()) {
											//if full,empty half
											if (in_fmap_buffer.size() == this->on_chip_size * 1024 / 8) {   // KB*1024/8 -> num
												for (int del_i = 0; del_i < in_fmap_buffer.size() / 2; del_i++) {
													in_fmap_buffer.erase(in_fmap_buffer.begin());
												}
											}
											in_fmap_buffer.insert(pair<unsigned int, address_t>((index_G + g)*this->group_size*_word_size + (index_K + k)*this->filter_size*_word_size + (index_R + r)*this->row_filter_size*_word_size + (index_S + s)*dnn_layer->get_C()*_word_size + (index_C + c), &filter_address[(index_G + g)*this->group_size*_word_size + (index_K + k)*this->filter_size*_word_size + (index_R + r)*this->row_filter_size*_word_size + (index_S + s)*dnn_layer->get_C()*_word_size + (index_C + c)]));

											std::stringstream ss;
											ss << dec2hex((long long int)(&filter_address[(index_G + g)*this->group_size*_word_size + (index_K + k)*this->filter_size*_word_size + (index_R + r)*this->row_filter_size*_word_size + (index_S + s)*dnn_layer->get_C()*_word_size + (index_C + c)])) << " " << "READ" << " " << std::to_string(this->sdmemoryStats.total_cycles) << " " << "weight";
											std::string result = ss.str();
											cpu->req_fifo.push(result);
											cpu->npu_num++;
						...
						for (unsigned c = 0; c < current_tile->get_T_C(); c++) {
							for (unsigned r = 0; r < current_tile->get_T_R(); r++) {
								for (unsigned s = 0; s < current_tile->get_T_S(); s++) {
									...
									unsigned index_S = current_S * this->current_tile->get_T_S();
									this->sdmemoryStats.n_SRAM_weight_reads++; //To track information
									data_t data = filter_address[(index_G + g)*this->group_size*_word_size + (index_K + k)*this->filter_size*_word_size + (index_R + r)*this->row_filter_size*_word_size + (index_S + s)*dnn_layer->get_C()*_word_size + (index_C + c)];

									//Shift of this weight is g*group_tile_size + k*filter_tile_size + c*filter_channel_tile_size + r*s_tile_size + s
									unsigned int receiver = g * current_tile->get_T_K()*window_size + k * window_size + c * current_tile->get_T_R()*current_tile->get_T_S() + r * current_tile->get_T_S() + s + folding_shift; //+1 because of the ms we leave free for the folding
					...
					for (int i = 0; i < this->current_tile->get_T_N(); i++) {
						for (int g = 0; g < this->current_tile->get_T_G(); g++) {
							for (int c = 0; c < this->current_tile->get_T_C(); c++) {
								...
								unsigned int in_fmap_num_index = (index_N + i)*this->input_size + ((index_X*this->dnn_layer->get_strides() + x) + index_R)*this->dnn_layer->get_Y()*this->dnn_layer->get_C()*this->dnn_layer->get_G()*_word_size + ((index_Y*this->dnn_layer->get_strides() + y) + index_S)*this->dnn_layer->get_C()*this->dnn_layer->get_G()*_word_size + (index_G + g)*dnn_layer->get_C()*_word_size + (index_C + c)*_word_size;
								data_t data = input_address[in_fmap_num_index];
								//if this is not in buffer
								DataPackage* pck = new DataPackage(sizeof(data_t), data, IACTIVATION, 0, MULTICAST, destination_vector, this->num_ms);

								//added by wangjs
								auto it = in_fmap_buffer.find(in_fmap_num_index);
								if (this->reuse_type == "IS") {
									if (it == in_fmap_buffer.end()) {
										//if full,empty half
										if (in_fmap_buffer.size() == this->on_chip_size * 1024 / 8) {
											for (int del_i = 0; del_i < in_fmap_buffer.size() / 2; del_i++) {
												in_fmap_buffer.erase(in_fmap_buffer.begin());
											}
										}
										in_fmap_buffer.insert(pair<unsigned int, address_t>(in_fmap_num_index, &input_address[in_fmap_num_index]));

										
										std::stringstream ss;
										ss << dec2hex((long long int)(&input_address[in_fmap_num_index])) << " " << "READ" << " " << std::to_string(this->sdmemoryStats.total_cycles) << " " << "feature_map";
										std::string result = ss.str();
										cpu->req_fifo.push(result);
										cpu->npu_num++;
										this->load_instruction++;
	...
	if (!write_fifo->isEmpty()) {
		...
		//std::cout << "receiving final data---------" << std::endl;
		...
		for (int i = 0; i < write_fifo->size(); i++) {
			...
			if (current_output_pixel >= output_pixels_to_compute * 0.5) {
...
void SDMemory::sendPackageToInputFifos(DataPackage* pck) {
	...
	if (pck->isBroadcast()) {
		...
		for (int i = 0; i < this->n_read_ports; i++) {
			...
			pck_new->setDelay(pck->getDelay());
			/*		pck_new->setAdded_cycle(pck->getAdded_cycle());
					pck_new->setAddr_to_dram(pck->getAddr_to_dram());*/
					//std::cout << pck_new->addr_to_dram << std::endl;
	...
	/*can not delete this, for subsequent use*/
...
void SDMemory::send() {
	...
	for (int i = 0; i < this->n_read_ports; i++) {
		...
		//If psums fifo is empty then input fifo is checked. If psum is not empty then else do not compute. Important this ELSE to give priority to the psums and do not send more than 1 pck
			...
			//std::cout << "mem:  " << pck->getAddr_to_dram() << std::endl;
			...
			if (pck->getIterationK() <= current_iteration) { //Check if the iteration of the package is older or equal to the current
				...
				//std::cout << "get delay: " << pck->getDelay() << std::endl;
...
void SDMemory::receive(CPU *cpu) { //TODO control if there is no space in queue
	...
	for (int i = 0; i < write_port_connections.size(); i++) { //For every write port
		...
		if (write_port_connections[i]->existPendingData()) {
			...
			for (int i = 0; i < data_received.size(); i++) {
				...
				DataPackage* pck = new DataPackage(data_received[i]);
				pck->setAdded_cycle(std::to_string(this->sdmemoryStats.total_cycles));
 
				//added by wangjs
				if (this->reuse_type == "OS") {
					auto it = in_fmap_buffer.find(std::stoul(pck->getAddr_to_dram()));
					//if this is not in buffer
					if (it == in_fmap_buffer.end()) {
						//if full,empty half
						if (in_fmap_buffer.size() == this->on_chip_size * 1024 / 8) {   // KB*1024/8 -> num
						...
						in_fmap_buffer.insert(pair<unsigned int, address_t>(std::stoul(pck->getAddr_to_dram()), &output_address[0])); //&STA_address[0] or others, it does not matter


						std::stringstream ss;
						ss << pck->getAddr_to_dram() << " " << pck->getReq_type() << " " << pck->getAdded_cycle() << " " << pck->getAddr_type();
						std::string result = ss.str();
						cpu->req_fifo.push(result);
						cpu->npu_num++;
						this->store_instruction++;
...
void SDMemory::getInputData(string data)
{
	...
	addr_mac = dec2hex(mac_flag.to_ulong() + MAC_DATA_BEGIN);
	//*****CTR*****//
	...
	addr_ctr = dec2hex(ctr_flag.to_ulong() + CTR_DATA_BEGIN);
	//*****BMT*****//
	...
	for (int index_i = 0; index_i < 6; index_i++)
	{
		...
		string str_temp2 = dec2hex(bmt_flag.to_ullong() + bmt_data_begn[index_i]);
...
unsigned long long SDMemory::get_Instruction()
{
	return this->load_instruction;
}

unsigned long long SDMemory::get_Instruction_S()
{
	return this->store_instruction;
}

unsigned int SDMemory::get_global_cycle()
{
	return this->sdmemoryStats.total_cycles;
}
