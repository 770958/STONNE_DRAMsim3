...
SparseSDMemory::SparseSDMemory(id_t id, std::string name, Config_stonne stonne_cfg, Connection* write_connection) : MemoryController(id, name) {
...
//added by wangjs
string dec2hex_(long long int i) //将int转成16进制字符串
{
...
unsigned long long clk_from_dram_last3 = 0;
unsigned long long clk_from_dram_now3 = 0;

void SparseSDMemory::cycle(CPU *cpu) {
		...
		for (int i = 0; i < this->configurationVNs.size(); i++) {
			...
			for (; j < this->configurationVNs[i].get_VN_Size(); j++) {
				...
				if (this->reuse_type == "WS") {
					auto it = in_fmap_buffer.find(sta_current_index_matrix + sub_address);
					...
					if (it == in_fmap_buffer.end()) {
						...
						in_fmap_buffer.insert(pair<unsigned int, address_t>(sta_current_index_matrix + sub_address, &STA_address[sta_current_index_matrix + sub_address]));

						std::stringstream ss;
						ss << dec2hex_((long long int)(&STA_address[sta_current_index_matrix + sub_address])) << " " << "READ" << " " << std::to_string(this->sdmemoryStats.total_cycles) << " " << "weight";
		...
		for (int j = init_point_str; j < end_point_str; j++) {   //For each element in the current vector in the str matrix
			...
			if (STR_metadata[str_current_index*STR_DIST_VECTOR + j * STR_DIST_ELEM]) {
				...
				if (this->reuse_type == "IS") {
					auto it = in_fmap_buffer.find(src);
					//if this is not in buffer
					if (it == in_fmap_buffer.end()) {
						//if full,empty half
						if (in_fmap_buffer.size() == this->on_chip_size * 1024 / 8) {   // KB*1024/8 -> num
						...
						in_fmap_buffer.insert(pair<unsigned int, address_t>(src, &STR_address[src]));

						std::stringstream ss;
						ss << dec2hex_((long long int)(&STR_address[src])) << " " << "READ" << " " << std::to_string(this->sdmemoryStats.total_cycles) << " " << "feature_map";
						std::string result = ss.str();
						cpu->req_fifo.push(result);
						cpu->npu_num++;
						this->sendPackageToInputFifos(pck);
			...
			// pck->setAdded_cycle(this->sdmemoryStats.total_cycles / 2);
		//	 pck->setAddr_to_dram(addr2sim);
			 //std::cout << "mem:  " << addr2sim << std::endl;
	...
	if (!write_fifo->isEmpty()) {
		...
		for (int i = 0; i < write_fifo->size(); i++) {
			...
			//data flows into the system,do not need to simu in dramsim 
	...
	if (current_state == CONFIGURING) {
		...
		if (sta_current_index_metadata >= this->dim_sta * 0.5) {
			...
			//std::cout << "sta_current_index_metadata: " << sta_current_index_metadata << " this->dim_sta: " << this->dim_sta << std::endl;
		}
	...
	this->send();
	//std::cout <<"send_finished: " <<send_finished <<"remaining: "<<reg_pck.size()<< std::endl;

	//std::cout << "reg_pck.size(): " << reg_pck.size() << std::endl;
}
	...
	if (pck->isBroadcast()) {
		...
		for (int i = 0; i < this->n_read_ports; i++) {
			...
			if (pck->get_data_type() == PSUM) { //Actually a PSUM cannot be broadcast. But we put this for compatibility
				...
				//std::cout << "fifo:  " << pck_new->getAddr_to_dram() << std::endl;
...
void SparseSDMemory::send() {
	...
	for (int i = 0; i < this->n_read_ports; i++) {
		...
		//If psums fifo is empty then input fifo is checked. If psum is not empty then else do not compute. Important this ELSE to give priority to the psums and do not send more than 1 pck
			...
			//std::cout << "send:  " << pck->getAddr_to_dram() << std::endl;
...
void SparseSDMemory::receive(CPU *cpu) { //TODO control if there is no space in queue
	//data from AS directly
...
void SparseSDMemory::getInputData(string data)
{
	...
	addr_mac = dec2hex_(mac_flag.to_ulong() + MAC_DATA_BEGIN);
	//*****CTR*****//
	...
	addr_ctr = dec2hex_(ctr_flag.to_ulong() + CTR_DATA_BEGIN);
	//*****BMT*****//
	...
	for (int index_i = 0; index_i < 6; index_i++)
	{
		...
		string str_temp2 = dec2hex_(bmt_flag.to_ullong() + bmt_data_begn[index_i]);
...
unsigned long long SparseSDMemory::get_Instruction()
{
	return this->load_instruction;
}

unsigned long long SparseSDMemory::get_Instruction_S()
{
	return this->store_instruction;
}

unsigned int SparseSDMemory::get_global_cycle()
{
