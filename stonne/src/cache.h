...
namespace dramsim3 {
	...
	/******************************************/
		...
		// Cache
		virtual void GetHitNum(char *address, dramsim3::CPU *cpu)=0;
		...
		void GetWrite(); //写入内存
		virtual void PrintCache(std::string file_name)=0;

		INIReader* reader_cache;
		unsigned long type_num[4] = { 0 }; //data access type from npu ----"cache[psum],cache[weight],dram[psum],dram[weight]"
	...
	/******************************************/
		void MSHR_cycle(bitset<32> flags, dramsim3::CPU *cpu);
		std::pair<bool, bool> WillAcceptMiss(bitset<32> flags);
		virtual void AddMiss2Entry(bitset<32> flags, int acc, dramsim3::CPU *cpu);
		void ReleaseMSHR(dramsim3::CPU *cpu);
	/******************************************/
