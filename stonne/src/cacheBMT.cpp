...
namespace dramsim3 {
	cacheBMT::cacheBMT(const std::string & config_file)
		:Cache(config_file)
	{
		reader_cacheBMT = new INIReader(config_file);
		InitCache();
		delete(reader_cacheBMT);
	}

	void cacheBMT::InitCache()
	{
		const auto& reader = *reader_cacheBMT;
		for (int i = 0; i < 6; i++) {
			this->type_num[i] = 0;
		}
		bit_block = 0;
		...
		bmt_length = reader.GetInteger("BMT", "bmt_length", 8); //Bytes
		hash_delay = reader.GetInteger("BMT", "hash_delay", 8); //cycles
		i_cache_size = reader.GetInteger("BMT", "i_cache_size", 256); //Cache size - 1,2,4,8,16,32,64...2^18 KB
		i_cache_line_size = reader.GetInteger("BMT", "i_cache_line_size", 64); //Cache Line size - 1,2,4,8,16,32,64...2^18 B
		t_assoc = reader.Get("BMT", "t_assoc", "SA");//mapping method - direct_mapped \ set_associative \ full_associative 


		i_cache_set = reader.GetInteger("BMT", "i_cache_set", 4); //lines_num in each set- 1,2,4,8,16,32,64...2^18
		t_replace = reader.Get("BMT", "replace_policy", "lru");//replace policy -  fifo\lru\lfu\random
		t_write = reader.Get("BMT", "write_policy", "WB");//write policy -  write through \ write_back
	...
	void cacheBMT::GetWrite(CPU *cpu)
	{
		//write back to dram 
		this->clk_cache += cpu->bmt_cache->hash_delay;
	...
	void cacheBMT::cache_clock(dramsim3::CPU *cpu)
	{
		string str_temp = cpu->ctr_data_row;
		for (int level = 0; level < 6; level++) {
			this->level = level;
			if (!cpu->bmt_fifo->isEmpty()) {
				if (this->get_next) {
					this->get_next = false;
					char* op_addr;
					unsigned long num = 0;
					...
					op_addr = new char[(str_temp.substr(0, 10)).length() + 1]; // 分配足够的内存空间，包括终止符'\0'
					(str_temp.substr(0, 10)).copy(op_addr, (str_temp.substr(0, 10)).length());
					op_addr[(str_temp.substr(0, 10)).length()] = '\0'; // 手动添加终止符'\0'
					...
					num = bmt_flag.to_ulong() / 16 % (cpu->block_length / cpu->bmt_length);
					bitset<14> bmt_offset(num);
					...
					string str_temp2 = cpu->Conversion_hex(bmt_flag.to_ullong() + bmt_data_begn[level]);
					cpu->bmt_fifo->front()->setAddr_to_dram(str_temp2);
					str_temp2 += " " + cpu->bmt_fifo->front()->getReq_type() + " " + cpu->bmt_fifo->front()->getAdded_cycle();
					cpu->bmt_data_row.push_back(str_temp2);
					str_temp = str_temp2;

					string op_type;
					/*bmt*/
					op_type = (str_temp2.substr(11, 4) == "READ") ? "l" : "s";
					op_type += "\t";
					op_type += str_temp2.substr(0, 10).c_str();
					address = new char[op_type.length() + 1]; // 分配足够的内存空间，包括终止符'\0'
					op_type.copy(address, op_type.length());
					address[op_type.length()] = '\0'; // 手动添加终止符'\0'

					this->addr = cpu->bmt_fifo->front()->getAddr_to_dram();
					this->operation = cpu->bmt_fifo->front()->getReq_type();
					this->added_cycle = cpu->bmt_fifo->front()->getAdded_cycle();
					this->type = cpu->bmt_fifo->front()->getAddr_type();

					DataPackage* del_pck_ptr = cpu->bmt_fifo->front();
					cpu->bmt_fifo->pop();
					delete del_pck_ptr;
				}
				if (std::stoi(this->added_cycle) <= this->clk_cache) {
					...
					else if (hit && is_store)
					{
						...
						if (cpu->act_mee == 1) {
							cpu->completeTime += cpu->bmt_cache->hash_delay;
						}
						this->get_next = true;
					}
					// not hit，load
	...
	void cacheBMT::AddMiss2Entry(bitset<32> flags, int acc, dramsim3::CPU *cpu)
	{
		/*
	...
	bool cacheBMT::IsHit(bitset<32> flags)
	{
		bool ret = false;
	...
	void cacheBMT::LruHitProcess() // if the replacement policy is LRU,and hit
	{
	...
	void cacheBMT::LruUnhitSpace() // if the replacement policy is LRU,and not hit,but there has a spaceline
	{
		for (i = (current_set*i_cache_set); i < ((current_set + 1)*i_cache_set); i++)
		{
	...
	void cacheBMT::GetRead(bitset<32> flags, CPU *cpu)
	{
		bool space = false;
	...
	void cacheBMT::GetReplace(bitset<32> flags, CPU *cpu)
	{
		if (t_replace == "random")
		{
	...
	std::pair<bool, bool> cacheBMT::WillAcceptMiss(bitset<32> flags)
	{
		/*
	...
	void cacheBMT::ReleaseMSHR(dramsim3::CPU *cpu)
	{
		for (int index = 0; index < capacity_mshr; index++)
		{
	...
	void cacheBMT::MSHR_cycle(bitset<32> flags, dramsim3::CPU *cpu)
	{
		int MSHR_type = 0;
	...
	void cacheBMT::PrintCache(string layer_name, string index_layer_, string file)
	{
		//std::cout << "Printing BMT cache" << std::endl;
		ofstream ofs;

		string bmt_out_file = file + "/bmtcache.csv";
		ofs.open(bmt_out_file, ios::out | ios::app);
	...
	/*
	void cacheBMT::PrintCache(string layer_name, string index_layer_)
	{
		std::cout << "Printing BMT cache" << std::endl;
		ofstream ofs;

		string bmt_out_file = "../dramsim3/output/bmtcache.csv";
		ofs.open(bmt_out_file, ios::out | ios::app );
