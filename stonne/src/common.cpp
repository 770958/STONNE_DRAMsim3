...
std::ostream& operator<<(std::ostream& os, const Command& cmd) {
    std::vector<std::string> command_string = {
        ...
        "self_refresh_exit",
        "WRONG"};
    os <<command_string[static_cast<int>(cmd.cmd_type)]<<"\t"
                      <<cmd.Channel() <<"\t" 
                      <<cmd.Rank() <<"\t"  
                      <<cmd.Bankgroup() <<"\t" 
                      <<cmd.Bank()<<"\t" 
...
std::ostream& operator<<(std::ostream& os, const Transaction& trans) {
    ...
    os << trans.addr<<"\t"<<trans_type;
    return os;
}
std::stringstream& operator>>(std::stringstream& is, Transaction& trans) {
	std::unordered_set<std::string> write_types = { "WRITE", "write", "P_MEM_WR",
							  "BOFF" };
	std::string mem_op;
	is >> std::hex >> trans.addr >> mem_op >> std::dec >> trans.added_cycle;
	//cout<<std::hex << trans.addr <<"\t"<< mem_op <<"\t"<<  std::dec << trans.added_cycle<<endl;
