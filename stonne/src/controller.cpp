    ...
    while (it != return_queue_.end()) {
        if (clk >= it->complete_cycle) {
            if (it->is_write) {
				...
				this->read_delay.push(clk_ - it->added_cycle);
            }
...
std::queue<int> Controller::ReturnDelay() { 
	std::queue<int> delay;
...
void Controller::FreeDelayQueue() {
	while (!this->read_delay.empty())
	{
		this->read_delay.pop();
	}
...
void Controller::IssueCommand(const Command &cmd) {
    ...
    if (cmd.IsRead()) {
        ...
        while (num_reads > 0) {
			...
			//this->completeTime = it->second.complete_cycle; //记录完成时间
