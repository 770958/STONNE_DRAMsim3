    ...
    : BaseDRAMSystem(config, output_dir, read_callback, write_callback) 
{
	...
	//std::cout << "JedecDRAMSystem IN" << std::endl;
    ctrls_.reserve(config_.channels);
    for (auto i = 0; i < config_.channels; i++) {

		//std::cout << "JedecDRAMSystem loop" << std::endl; 
		ctrls_.push_back(new Controller(i, config_, timing_));
		//std::cout << "JedecDRAMSystem loop out" << std::endl;
    }
	//std::cout << "JedecDRAMSystem OUT" << std::endl;
}
...
bool JedecDRAMSystem::AddTransaction(uint64_t hex_addr, bool is_write, std::string addr_str) {
// Record trace - Record address trace for debugging or other purposes
   ...
   // std::cout<<addr_str<<std::endl;
    ...
    assert(ok);
    if (ok) {
        Transaction trans = Transaction(hex_addr, is_write, addr_str);
...
void JedecDRAMSystem::ClockTick() {
    ...
    for (size_t i = 0; i < ctrls_.size(); i++) {
        ctrls_[i]->ClockTick();
		std::queue<int> delay;
		delay = ctrls_[i]->ReturnDelay();
		while (!delay.empty())
		{
			this->read_delay.push(delay.front());
			delay.pop();
		}
 		//ctrls_[i]->FreeDelayQueue();
    }
    clk_++;
	
	//Manually comment out this code
    /*if (clk_ % config_.epoch_period == 0) {
        PrintEpochStats();
    }*/
    return;
...
std::queue<int> JedecDRAMSystem::ReturnDelay() {
	std::queue<int> delay;
...
void JedecDRAMSystem::FreeDelayQueue() {
	while (!this->read_delay.empty()) {
		this->read_delay.pop();
	}
}
...
bool IdealDRAMSystem::AddTransaction(uint64_t hex_addr, bool is_write, std::string addr_str) {
...
std::queue<int> IdealDRAMSystem::ReturnDelay() { return std::queue<int>(); }

void IdealDRAMSystem::FreeDelayQueue() {
	while (!this->read_delay.empty()) {
		this->read_delay.pop();
	}
}
