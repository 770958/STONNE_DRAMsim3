...
void configConvParameters(int argc, char *argv[], Config_stonne &stonne_cfg, std::string &layer_name, unsigned int &R, unsigned int &S, unsigned int &C, unsigned int &K, unsigned int &G, unsigned int &N, unsigned int &X, unsigned int &Y, unsigned int &strides,
...
void configSparseGEMMParameters(int argc, char *argv[], Config_stonne &stonne_cfg, std::string &layer_name, unsigned int &M, unsigned int &N, unsigned int &K, unsigned int &MK_sparsity, unsigned int &KN_sparsity, Dataflow &dataflow, unsigned int &optimize);

void configDenseGEMMParameters(int argc, char *argv[], Config_stonne &stonne_cfg, std::string &layer_name, unsigned int &M, unsigned int &K, unsigned int &N, unsigned int &T_M, unsigned int &T_K, unsigned int &T_N);

void configSparseDenseParameters(int argc, char *argv[], Config_stonne &stonne_cfg, std::string &layer_name, unsigned int &M, unsigned int &N, unsigned int &K, unsigned int &MK_sparsity, unsigned int &T_N, unsigned int &T_K);
...
bool runConvCommand(int argc, char *argv[]) {
	...
	Config_stonne stonne_cfg; //Hardware parameters
	//stonne_cfg.m_MSNetworkCfg.ms_size=128;
	//Modify stonne_cfg and the variables according to user arguments
    configConvParameters(argc, argv, stonne_cfg, layer_name, R, S, C, K, G, N, X, Y, strides, T_R, T_S, T_C, T_K, T_G, T_N, T_X_, T_Y_); 
    ...
    stonne_instance->run(layer_name); //Running the simulator 
