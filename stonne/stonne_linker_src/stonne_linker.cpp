...
void simulateDenseConvForward(std::string layer_name, float* input, float* weight, float* output, int R, int S, int C, int K, int G, int N, int X, int Y, int X_, int Y_, int strides, int pad_x, int pad_y, std::string path_to_tile, Config_stonne stonne_cfg) {
...
*/
   ...
   stonne_instance->run(layer_name); //Running the accelerator and generates the output in ofmap_raw
...
void simulateSparseGemmForward(std::string layer_name, float* KN_matrix_raw, float* MK_matrix_raw, float* output_raw, int N, int G, int gemm_M, int gemm_K, int gemm_N, float sparsity_level, Config_stonne stonne_cfg, Dataflow dataflow) {
...
void simulateSparseGemmWithBitmapsForward(std::string layer_name, float* KN_matrix_raw, float* MK_matrix_raw, float* output_raw, int N, int G, int gemm_M, int gemm_K, int gemm_N, unsigned int* MK_bitmap, unsigned int* KN_bitmap, Config_stonne stonne_cfg, Dataflow dataflow) {
...
void simulateDenseGemmForward(std::string layer_name, float* KN_matrix_raw, float* MK_matrix_raw, float* output_raw, int N, int G, int gemm_M, int gemm_K, int gemm_N, std::string path_to_tile, Config_stonne stonne_cfg) {
   if(path_to_tile == "") {
       std::cout << "Tile file parameters must be specified" << std::endl;
       exit(1);
   }
